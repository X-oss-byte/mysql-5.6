...
class mysqlServer(Server):
    ...
    def __init__( self, name, server_manager, code_tree, default_storage_engine
        ...
        self.langstring = f"--lc-messages-dir={self.langdir}"
        self.preferred_base_port = 9306
        if self.version.startswith('5.1') or self.version.startswith('5.0'):
            self.langstring = f"--language={os.path.join(self.langdir, 'english')}"
    ...
    def get_start_cmd(self):
        ...
        server_args = [
            ...
            f"--character-sets-dir={self.charsetdir}",
            self.langstring,
            f"--ssl-ca={os.path.join(self.std_data, 'cacert.pem')}",
            f"--ssl-cert={os.path.join(self.std_data, 'server-cert.pem')}",
            f"--ssl-key={os.path.join(self.std_data, 'server-key.pem')}",
