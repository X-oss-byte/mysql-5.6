...
class mysqlServer(Server):
    ...
    def __init__( self, name, server_manager, code_tree, default_storage_engine
        ...
        self.dirset = {
            f'var_{self.name}': {
                ...
                'run': None,
                'tmp': None,
                'master-data': {'test': None, 'mysql': None},
            }
        }
    ...
    def initialize_databases(self):
        ...
        if not self.bootstrap_cmd:
            mysqld_args = [
                "--no-defaults",
                "--bootstrap",
                f"--basedir={self.code_tree.basedir}",
                f"--datadir={self.datadir}",
                "--loose-skip-falcon",
                "--loose-skip-ndbcluster",
                f"--tmpdir={self.tmpdir}",
                "--core-file",
                f"--lc-messages-dir={self.langdir}",
                f"--character-sets-dir={self.charsetdir}",
            ]
        ...
        with open(self.bootstrap_log,'w') as bootstrap_log:
            with open(self.bootstrap_file,'r') as bootstrap_in:
        ...
        if bootstrap_retcode := bootstrap_subproc.returncode:
            self.logging.error(
                f"Received retcode: {bootstrap_retcode} executing command: {self.bootstrap_cmd}"
            )
            self.logging.error(f"Check the bootstrap log: {self.bootstrap_log}")
            sys.exit(1)
    ...
    def get_start_cmd(self):
       ...
       # We make ourselves bug-compatible with MTR / xtrabackup
        ...
        if self.system_manager.wsrep_provider_path:
            wsrep_provider_string = (
                f"--wsrep_provider={self.system_manager.wsrep_provider_path}"
            )
        # handle wsrep_cluster_address argument
        wsrep_cluster_string = f"--wsrep_cluster_address='gcomm://{self.wsrep_cluster_addr}{self.listen_addr_string}'"
        for server_opt in self.server_options:
            if server_opt.startswith("--wsrep_cluster_address="):
                ...
                # what is going on
                if '?gmcast.listen_addr' not in server_opt:
                    server_opt = server_opt.strip()+ listen_addr
        server_args = [
            self.process_server_options(),
            f"{wsrep_cluster_string}",
            f"{wsrep_provider_string}",
            "--wsrep_debug=ON",
            "--wsrep_provider_options='ist.recv_addr=%s:%d'"
            % (self.ip_address, self.galera_recv_port),
            ...
            f'--log-bin={os.path.join(self.logdir, "mysqld-bin")}',
            "--binlog-direct-non-transactional-updates",
            "--binlog-format=ROW",
            "--general_log=1",
            f"--general_log_file={self.general_log_file}",
            "--slow_query_log=1",
            f"--slow_query_log_file={self.slow_query_log_file}",
            f"--basedir={self.code_tree.basedir}",
            f"--datadir={self.datadir}",
            f"--tmpdir={self.tmpdir}",
            f"--character-sets-dir={self.charsetdir}",
    ...
    def gen_cnf_file(self, server_args):
        ...
        with open(self.cnf_file,'w') as config_file:
